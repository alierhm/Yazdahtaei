...
Game::Game(string_view numberOfPlayers) {
    setNumberOfPlayers_Usernames_And_Passwords(numberOfPlayers);
...
int Game::play(Player &player) {
    ...
    for (;;) {
        ...
        cout << format("Enter a number between 1 and {}: ", player.howManyCardsInHand());
    ...
    int actualNumberOfTheChosenCard{player.getIthCardInHand(stoi(cardNumberInputFromUser)).getNumber()};
    if (!m_background.empty()) {
        if (actualNumberOfTheChosenCard >= 11) {
            if (actualNumberOfTheChosenCard == 11) {
                ...
                if (numberOfJacksOrNumbersInBackground >= 0) {
                    ...
                    for (auto &backCard: m_background.getStack()) {
                        if (backCard.getNumber() <= 11) { matchForJack.push_back(backCard); }
                    }
            ...
            vector<vector<Card> > combinationsOfSize1FromBackground{m_background.getCombinationsOfSize(1)};
            for (auto &combination: combinationsOfSize1FromBackground) {
                int sumOfNumberOfCardsInCombination{sumOfNumberOfCards(combination)};
                if ((actualNumberOfTheChosenCard + sumOfNumberOfCardsInCombination) == 11) {
                    matches.push_back(combination);
                }
            }
            if (m_background.size() >= 2) {
                vector<vector<Card> > combinationsOfSize2FromBackground{m_background.getCombinationsOfSize(2)};
                ...
                if (m_background.size() >= 3) {
                    vector<vector<Card> > combinationsOfSize3FromBackground{m_background.getCombinationsOfSize(3)};
                    ...
                    if (m_background.size() >= 4) {
                        vector<vector<Card> > combinationsOfSize4FromBackground{m_background.getCombinationsOfSize(4)};
                            ...
                            if (m_background.size() >= 6) {
                                ...
                                if (m_background.size() >= 7) {
                                    ...
                                    if (m_background.size() >= 8) {
                                        ...
                                        for (auto &combination: combinationsOfSize8FromBackground) {
                                            int sumOfNumberOfCardsInCombination{sumOfNumberOfCards(combination)};
                                            if ((actualNumberOfTheChosenCard + sumOfNumberOfCardsInCombination) == 11) {
                                                matches.push_back(combination);
                                            }
        ...
        if (matches.empty()) {
            ...
            m_background.push_back(player.erase_Ith_Card_In_Hand(stoi(cardNumberInputFromUser)));
        } else {
            ...
            for (auto &match: matches) {
                ...
                cout << format("{}{} match: ", numOfMatch,
                               (numOfMatch == 1) ? "st" : (numOfMatch == 2) ? "nd" : (numOfMatch == 3) ? "rd" : "th");
                for (auto &card: match) {
                    card.toString();
                    cout << " ";
                }
            ...
            while (!validInputToChooseAMatch) {
                // get input from player to choose a match
                cout << format("Choose a match to collect the cards (enter a number between 1 and {}): ",
                               matches.size());
            ...
            cardsToCollect.push_back(player.erase_Ith_Card_In_Hand(stoi(cardNumberInputFromUser)));
...
void Game::calculateScores() {
    ...
    for (int i{0}; i < m_players.size(); ++i) {
        for (int j{0}; j < m_players.at(i).getCollectedCards().size(); ++j) {
            ...
            if (m_players.at(i).getCollectedCards().at(j).getNumber() == 11 || m_players.at(i).getCollectedCards().at(j)
                .getNumber() == 1) {
                m_players.at(i).addScore(1);
            }
            if (m_players.at(i).getCollectedCards().at(j).getNumber() == 10 && m_players.at(i).getCollectedCards().at(j)
                .getSuit() == "Diamonds") {
                m_players.at(i).addScore(3);
            }
            if (m_players.at(i).getCollectedCards().at(j).getNumber() == 2 && m_players.at(i).getCollectedCards().at(j).
                getSuit() == "Clubs") {
    ...
    for (int h{0}; h < players_copy.size() - 1; ++h) {
        for (int i{0}; i < players_copy.size() - 1 - h; ++i) {
            if (players_copy.at(i).getSurCount() > players_copy.at(i + 1).getSurCount()) {
                swap(i, i + 1);
            }
        }
    }
    for (auto &player: m_players) {
        player.addScore((player.getSurCount() - players_copy.at(0).getSurCount()) * 5);
    }
}

void Game::setNumberOfPlayers_Usernames_And_Passwords(string_view numberOfPlayers) {
...
void Game::initializeRepositoryAndBackground() {
    ...
    for (;;) {
        ...
        if (jacksCount >= 2 || queensCount > 2 || kingsCount > 2) {
            if (numberOfTimes1JackOccurred == 1) { numberOfTimes1JackOccurred = 0; }
            for (int i{0}; i < 4; ++i) {
        ...
        if (jacksCount == 1) {
            if (numberOfTimes1JackOccurred == 0) { numberOfTimes1JackOccurred = 1; } else if (
...
void Game::initializePlayers(string_view numberOfPlayers) {
    if (numberOfPlayers == "2" || numberOfPlayers == "3") {
        while (m_players.size() < m_numberOfPlayers) {
            string name;
            while (true) {
                cout << format("Enter the name of player {}: ", m_players.size() + 1);
                getline(cin, name);
                if (!name.empty()) { break; } else {
                    cout << "Invalid input. Please enter a name." << endl;
                }
            }
            Player player(m_players.size() + 1, name);
...
void Game::runTheMainPartOfTheGame() {
    while (true) {
        ...
        if (maximScore >= 62 && m_repository.empty()) {
        ...
        for (int o{0}; o < 4; ++o) {
            for (int u{0}; u < m_players.size(); ++u) {
                ...
                if (tempId != 0) { m_idOfLastPlayerThatCollectedSomeCards = tempId; }
            }
        }
        if (m_repository.empty() && !m_background.empty()) {
            vector<Card> backgroundCardsAfterClearing(m_background.clear());
            m_players.at(m_idOfLastPlayerThatCollectedSomeCards - 1).insertCollectedCards(backgroundCardsAfterClearing);
            calculateScores();
        ...
        if (maxScore >= 62 && m_repository.empty()) {
            if (m_numberOfPlayers == 2 || m_numberOfPlayers == 3) {
                vector<int> scores(m_numberOfPlayers);
                for (auto &player: m_players) {
                    scores.push_back(player.getScore());
                ...
                scoreOfTeam1 = m_players.at(0).getScore() + m_players.at(2).getScore();
                scoreOfTeam2 = m_players.at(1).getScore() + m_players.at(3).getScore();
                if (scoreOfTeam1 > scoreOfTeam2 || scoreOfTeam2 > scoreOfTeam1) { break; } else { continue; }
            }
        }
    }
}
...
void Game::calculateRanks() {
    ...
    for (int e{0}; e < m_players.size() - 1; ++e) {
        for (int r{0}; r < m_players.size() - 1 - e; ++r) {
            if (playersToRankByScore.at(r)->getScore() > playersToRankByScore.at(r + 1)->getScore()) {
